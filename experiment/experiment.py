...
class Experiment:
    ...
    def run(self, num_generations: int):
        ...
        for i, key in enumerate(keys):
            key, subkey = jax.random.split(key)
            (state, params, problem_state), metrics = self._step((state, params, problem_state), key)
            # move this chunk to CPU immediately and drop device refs
            ...
            if isinstance(self._problem, TorchVisionProblem):
                print(
                    f"Generation {(i + 1)}"
                    f" | Mean fitness (Test): {float(metrics['mean_fitness_in_generation_test']):.2f}"
                    f" | Mean Accuracy (Test): {float(metrics['mean_accuracy_in_generation_test']):.2f}")
            else:
                print(
                    f"Generation {(i + 1)}"
                    f" | Mean fitness (Test): {float(metrics['mean_fitness_in_generation_test']):.2f}")
        # Optionally stack metrics if needed for downstream analysis
        metrics = jax.tree_util.tree_map(lambda *xs: np.concatenate(xs, axis=0), *collected_metrics)
    ...
    def _write_gif_best_running_visualization(self, key, state, problem_state):
        ...
        # eval best policy
        fitness, problem_state, info = self._problem.eval(
            subkey, jax.tree.map(lambda x: x[None], mean), problem_state
        )

        if isinstance(self._problem, BraxProblem):
            # force completion and pull to host once
            _ = jax.block_until_ready(fitness)
            ps = jax.device_get(info["env_states"].pipeline_state)

            rollout = [
                jax.tree_util.tree_map(lambda x: x[0, 0, t], ps)
